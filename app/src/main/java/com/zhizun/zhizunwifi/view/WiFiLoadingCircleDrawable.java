...
public class WiFiLoadingCircleDrawable extends WiFiLoadingDrawable {
    ...
    private int fSweepAngle = 0;

    public WiFiLoadingCircleDrawable() {
        super();
    }

    public WiFiLoadingCircleDrawable(int minSize) {
        super(minSize);
    }
    ...
    protected void drawForeground(Canvas canvas, Paint foregroundPaint) {
        if (!isFinsh){
            ...
            canvas.drawArc(mForegroundOval, -90,fSweepAngle, false, foregroundPaint);
        }
    }
}
