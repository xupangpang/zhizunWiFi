...
//				wifiCong.preSharedKey = pwd;//WPA-PSK密码
	 ...
	 * @return
	 */
	public int AddOpenWifiConfig(int netId, String ssid, String pwd, String bssid) {
		...
		return localWifiManager.addNetwork(config);// 将配置好的特定WIFI密码信息添加,添加完成后默认是不激活状态，成功返回NetID，否则为-1
	}
...
//	}
    ...
    //得到连接的信号强度
	public int getConnectedIntensity(WifiInfo curWifiInfo) {
		return (curWifiInfo == null) ? null : curWifiInfo.getRssi();
	}
	...
	public boolean getConnectedEncryption() {
		...
		for (ScanResult scResult : list) {
			if (!TextUtils.isEmpty(scResult.SSID) && scResult.SSID.equals(getConnectedSSID(getConnectionInfo()))) {
	...
	//得到连接的IP地址
	public int getConnectedMaxSpeed(WifiInfo curWifiInfo) {
		return (curWifiInfo == null) ? 0 : curWifiInfo.getLinkSpeed();
	}
...
//					continue;
		...
		for (int i = 0; i < mWifiConfigurations.size(); i++) {
			...
			if (!isEsixt) {
				...
				for (int j = 0; j < scanWiFiResults.size(); j++) {
					if (wifiConfiguration.SSID.replaceAll("\"", "")
							.equals(scanWiFiResults.get(j).SSID.replaceAll("\"", ""))) {
						mFreeWifiList.add(new FreeWifi(wifiConfiguration.SSID, wifiConfiguration.preSharedKey, wifiConfiguration.BSSID, Math.abs(scanWiFiResults.get(j).level), wifiConfiguration.networkId, !wifiConfiguration.wepKeys[0].contains("WPA")));
						scanWiFiResults.remove(j);
					}
				}
			}
...
//			}
	 ...
	 */
	public boolean checkIsCurrentWifiHasPassword(String currentWifiSSID, Context context) {
		try {
