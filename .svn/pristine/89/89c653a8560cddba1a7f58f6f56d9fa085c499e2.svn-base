...
public class TasksCompletedView extends View {
	...
	private void initAttrs(Context context, AttributeSet attrs) {
		...
		mRadius = typeArray.getDimension(R.styleable.TasksCompletedView_radius,
				80);
		TextPaintText = typeArray.getString(R.styleable.TasksCompletedView_text);
	...
	protected void onDraw(Canvas canvas) {
		...
		if (mProgress > 0) {
			...
			canvas.drawArc(oval, -90,
					...
					mRingPaint); //
			String txt = nt.format((float) mProgress / mTotalProgress);
			mTxtWidth = mTextPaint.measureText(txt, 0, txt.length());
			canvas.drawText(txt, mXCenter - mTxtWidth / 2, mYCenter
					+ mTxtHeight / 4, mTextPaint);
		}else {
			mTxtWidth = mTextPaint.measureText(TextPaintText, 0, TextPaintText.length());
			canvas.drawText(TextPaintText, mXCenter - mTxtWidth / 2, mYCenter
					+ mTxtHeight / 4, mTextPaint);
		}
	}
	...
	public void setmTotalProgress(long TotalProgress) {
	...
	public void setmProgress(long Progress) {
		postInvalidate();
		mProgress = Progress;
	}
}
