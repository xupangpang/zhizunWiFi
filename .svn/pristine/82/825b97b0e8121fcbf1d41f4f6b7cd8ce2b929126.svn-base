...
public class DhDB {
	...
	private final int COMLPETE_QUERYLIST = 0X110;
	SQLiteDatabase db;
	 ...
	 */
	@SuppressWarnings("unchecked")
	public <T> List<T> queryListAndMessage(SqlProxy proxy) {
		Cursor cursor = db.rawQuery(proxy.getSql(), proxy.paramsArgs());
		try {
			List<T> list = new ArrayList<T>();
			while (cursor.moveToNext()) {
			...
			if(mHandler != null){
				mHandler.sendEmptyMessage(COMLPETE_QUERYLIST);
			}
			return list;
		} catch (Exception e) {
			e.printStackTrace();
	 ...
	 * @param sql
	 * @param args
	 * @param handler 消息通知查询
	 * @return
	 */
	public <T> List<T> queryListAndMessage(Class<T> clazz, String where, Handler handler,
			Object... whereargs) {
		checkOrCreateTable(clazz);
		SqlProxy proxy = SqlProxy.select(clazz, where, whereargs);
		Log.i("LockService", "查询语句= " + proxy.getSql());
		if(mHandler == null){
			this.mHandler = handler;
		}
		return queryListAndMessage(proxy);
	}
	

	public <T> List<T> queryAll(Class<T> clazz) {
		checkOrCreateTable(clazz);
		SqlProxy proxy = SqlProxy.select(clazz, null, new  Object[]{});
		return queryList(proxy);
	}
	 ...
	 * @return
	 *//*
	public <T> List<T> queryAllAndMessage(Class<T> clazz, Handler handler) {
		checkOrCreateTable(clazz);
	...
	private <T> T cursorToBean(Cursor cursor, Class<T> clazz) {
		...
		for (Iterator<String> iterator = keys.iterator(); iterator.hasNext();) {
			...
			} else if (field.getType().equals(byte[].class)) { // 查询bitmap数据类型
				BeanUtil.setProperty(obj, key, cursor.getBlob(cursor.getColumnIndex(column)) );
			}
		}
	...
	private static String getCreatTableSQL(Class<?> clazz) {
		...
		for (Iterator<String> iterator = keys.iterator(); iterator.hasNext();) {
			...
			if (dataType == int.class || dataType == Integer.class
				...
				sql.append(" NUMERIC");
			} else if (dataType == byte[].class) { // 存储Bimap数据
				sql.append(" BLOB");
			}
